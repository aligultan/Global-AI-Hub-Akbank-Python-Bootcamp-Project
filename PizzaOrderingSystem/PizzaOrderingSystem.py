...
class Pizza():
    def __init__(self, aciklama,
                 fiyat):  # Ust sınıf olusturduk bunlardan sonraki pizza sınıfları super init fonksiyonuyla bu pizzalarin aciklama ve fiyatlarini alicak
...
class Klasik_Pizza(
    Pizza):  # parantezin içine pizza yazmamızın sebebi class özelliklerini ana sınıftan alması(Pizza sınıfı)
    def __init__(self):
...
def main():
    ...
    toplam_tutar = round(toplam_tutar,
                         2)  # Fiyatın virgülden sonraki kısmının sadece ilk 2 basamağını göstermesi için round fonksiyonunu kullandık.
